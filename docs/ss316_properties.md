...
Also: most primary sources (BS EN 10088-1, SSINA Table 5) only publish elevated-temperature data up to 500 °C. Values at 600 °C come from a different but compatible primary table, INCO Publication 2980, and are explicitly flagged.
...
| Poisson's Ratio | ν | ~0.30 (data scatters 0.26–0.34, T-dependent⁵) | | | | n/a | n/a |
...
³ Source tabulates this for 316L (UNS S31603) at 20/200/400 °C. Standard engineering practice treats 316 and 316L as identical for thermal conductivity. The difference between them is carbon content, which does not meaningfully affect k. The 600 °C value is from INCO 2980, which IS 316-specific.
...
**Avoid continuous service in 427–871 °C** (800–1600 °F). In this range, carbide precipitation in unstabilized 316 degrades corrosion resistance. Use 316L or 316Ti for continuous service in this window.
...
- **α is a mean coefficient referenced to 20 °C**, suitable for total ΔL calculations from a 20 °C reference state. For instantaneous α(T) in transient thermal FEA, differentiate the mean curve or use a finer-resolution reference table.
